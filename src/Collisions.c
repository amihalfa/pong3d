    ...
    if (ball->position.x > racket->position.x - rckt_width_mi && ball->position.x < racket->position.x + rckt_width_mi) {
        ...
        if (ball->position.y + ball->radius > racket->position.y - racket->radius) {
            ...
            if (there_is_a_collision) {
                ...
                if (fabs(ball->speed.y) < BALL_MIN_SPEED)
                    ball->speed.y = ((ball->speed.y < 0)? -1.0f : 1.0f) * BALL_MIN_SPEED;
            }
        }
    }
    return 0;
...
int collision_ball_ball(Ball* ball_a, Ball* ball_b) {
    ...
    if (dist_bb < dist) {
        ...
        if (fabs(ball_a->speed.y) < BALL_MIN_SPEED)
                    ball_a->speed.y = ((ball_a->speed.y < 0)? -1.0f : 1.0f) * BALL_MIN_SPEED;
        if (fabs(ball_b->speed.y) < BALL_MIN_SPEED)
                    ball_b->speed.y = ((ball_b->speed.y < 0)? -1.0f : 1.0f) * BALL_MIN_SPEED;
